...
func NewUserTokenRepo(data *Data, logger log.Logger) biz.IIdentityRepo {
	return &identityRepo{
		tokenCache: data,
...
func (repo *identityRepo) GetUserIdentity(ctx context.Context, token string) (int64, error) {
	uid_string, err := repo.tokenCache.rdb.Get(token).Result()
	if err != nil {
		return 0, err
	}
...
func (repo *identityRepo) AddUserIdentity(ctx context.Context, identity *biz.Identity) error {
	err := repo.tokenCache.rdb.Set(identity.Token, identity.Uid, 0).Err()
	if err != nil {
		return err
	}
	return nil
...
func (repo *identityRepo) DelUserIdentity(ctx context.Context, identity *biz.Identity) error {
	err := repo.tokenCache.rdb.Del(identity.Token).Err()
	if err != nil {
		return err
	}
	return nil
